...
def get_address():
    ...
    addr_fields = ['street', 'landmark', 'area', 'city', 'state', 'country', 'pincode']
    cols = range(5, 12) # індекси колонок у data.csv

    # Збираємо значення по кожній колонці один раз
    col_to_vals = {c: [] for c in cols}
        ...
        for row in rdr:
            for c in cols:
                if c < len(row):
                    val = row[c].strip()
                    if val:
                        col_to_vals[c].append(val)

    # Формуємо адресу - якщо колонка порожня, ставити запасне значення
    picked = []
    for c in cols:
        vals = col_to_vals[c]
        picked.append(choice(vals) if vals else 'N/A')

    return dict(zip(addr_fields, picked))
